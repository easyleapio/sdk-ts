...
import TokensInfo from "@/constants/tokens.json";
import TokensInfoSepolia from "@/constants/tokens.sepolia.json";
...
export function useBalance(props: UseBalanceProps): UseBalanceResult {
  ...
  const { mode, source, addressSource, addressDestination, chainIdSN } =
    useAccount();
  ...
  const sourceTokenAddr = useMemo(() => {
    ...
    return tokenInfo.length
      ? (tokenInfo[0].l1_token_address as `0x${string}`)
      : "0x0";
